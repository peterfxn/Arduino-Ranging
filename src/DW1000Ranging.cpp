...
//Here our handlers
//they are defined and attached in .h file and .ino file.
...
void DW1000RangingClass::startAsAnchor(char address[], const byte mode[], const bool randomShortAddress) { 
    //TODO: add data member to map lables with their latest range received
    //save the address
...
void DW1000RangingClass::loop() {
	...
	checkForReset(); //if no need to reset, (_sentAck | _receivedAck) is true
                     //also, (_sentAck && _receivedAck) is always false
	uint32_t time = millis(); // TODO other name - too close to "timer"
	if(time-timer > _timerDelay) {
		timer = time;
		timerTick(); //check for needs to add new devices by sending a BLINK message
	}
	...
	if(_sentAck) {
        ...
        if(messageType == MEAN_RANGE){
            visualizeDatas(data);
        }
        
		if(messageType != POLL_ACK && messageType != POLL && messageType != RANGE)//RANGE_REPORT is only received
			return;
		
		//A msg was sent. We launch the ranging protocole when a message was sent
		if(_type == ANCHOR) {
			if(messageType == POLL_ACK) { //if a POLL_ACK is sent by ANCHOR, update timePollAckSent of the device it sends to
				DW1000Device* myDistantDevice = searchDistantDevice(_lastSentToShortAddress);
				
				if (myDistantDevice) { //if the device is found
					DW1000.getTransmitTimestamp(myDistantDevice->timePollAckSent);//load TX_TIME register to myDistantDevice
				}
			}
		}
		else if(_type == TAG) {
			if(messageType == POLL) { //if a POLL is sent by a TAG, update one or all (depending on _lastSentToShortAddress) devices' timePollSent variable
				DW1000Time timePollSent;
				DW1000.getTransmitTimestamp(timePollSent); //load TX_TIME register to timePollSent
			...
			else if(messageType == RANGE) { //if a RANGE is sent by TAG, update one or all devices' timeRangeSent variable
	...
	if(_receivedAck) {
		...
		if(messageType == BLINK && _type == ANCHOR) { //if we receive a BLINK from TAG, parse BLINK message and add new TAG
			byte address[8];
			byte shortAddress[2];
			_globalMac.decodeBlinkFrame(data, address, shortAddress); //data stores blink frame.
                                                                      //frame[0] is a global constant for control
                                                                      //frame[1] is sequenceNumber
                                                                      //frame[2-8] is reversed address of the sender
                                                                      //frame[9-10] is reversed short address of the sender
                                                                      //decodeBlinkFrame transfers address and shortAddress into its parameters in correct order
		
            //we crate a new device with th tag
			DW1000Device myTag(address, shortAddress);
			
			if(addNetworkDevices(&myTag)) { //add the tag that sends the blink message to our devices and reply with a RANGING_INIT message
				if(_handleBlinkDevice != 0) {
					(*_handleBlinkDevice)(&myTag);
				}
				//we reply by the transmit ranging init message
				transmitRangingInit(&myTag); //generates a longMACFrame.
                                             //frame[0-4]: varaibles and constants for control 
                                             //frame[5-13]: destination address
                                             //frame[14-15]: source address
				noteActivity();
			}
			_expectedMsgId = POLL;
		}
		else if(messageType == RANGING_INIT && _type == TAG) { //if we receive a RANGING_INIT from ANCHOR, parse the message and add new ANCHOR
			
			byte address[2];
			_globalMac.decodeLongMACFrame(data, address); //decodes longMACFrame sent by the ANCHOR in RANGE_REPORT message       
                                                          //now address stores the short address of the sending ANCHOR
			//we crate a new device with the anchor
			DW1000Device myAnchor(address, true);
			
			if(addNetworkDevices(&myAnchor, true)) { //add new ANCHOR to our devices
            ...
            //we have a POLL, POLL_ACK, RANGE, RANGE_REPORT, or RANGE_FAILED message
            //shortMACFrame[0-4]: variables and constants for control
            //shortMACFrame[5-6]: destination address
            //shortMACFrame[7-8]: source address
            //data[SHORT_MAC_LEN]: message type
            //data[SHORT_MAC_LEN+1]: number of devices
			byte address[2];
			...
			if(_type == ANCHOR) {
				if(messageType != _expectedMsgId && messageType != MEAN_RANGE) {
					// unexpected message, start over again (except if already POLL)
					_protocolFailed = true;
				}
				if(messageType == POLL) { //if we receive a POLL from TAG, transmit POLL_ACK
				...
				else if(messageType == RANGE) { //if we receive a RANGE from TAG, calculate distance and transmit RANGE_REPORT
					...
					for(uint8_t i = 0; i < numberDevices; i++) {
						...
						if(shortAddress[0] == _currentShortAddress[0] && shortAddress[1] == _currentShortAddress[1]) {
							...
							if(!_protocolFailed) { //get necessary timestamps to calculate distance
				...
				if(messageType == POLL_ACK) { //if we receive a POLL_ACK from AHCNOR, update timestamps and reansmit RANGE
				...
				else if(messageType == RANGE_REPORT) { //if we receive a RANGE_REPORT, save the range
                ...
                else if(messageType == MEAN_RANGE){
                    visualizeDatas(data);
                    //TODO: handle MEAN_RANGE message here
                    //update two projRanges and calculate coordinates of tag if the protocal is active
                    _protocolFailed = false;
                    int index = data[SHORT_MAC_LEN+9];
                    if(index == 1){
                        projRange1 = data[SHORT_MAC_LEN + 1];
                    }
                    if(index == 2){
                        projRange2 = data[SHORT_MAC_LEN + 1];
                    }
                    Serial.print(projRange2);Serial.print("\n");
                }
			}
		}
